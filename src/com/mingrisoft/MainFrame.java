...
public class MainFrame extends JFrame {// 主窗体
	...
	private JLabel stateLabel = null;// 选定窗体状态标签(显示当前窗体信息
	private JLabel nameLabel = null;// “公司名称”标签(显示版权公司信息
	private JLabel nowDateLabel = null;// “当前日期”标签(显示当前日期信息
	private static JLabel czyStateLabel = null;// “操作员”标签(显示登录用户名信息
	...
	public static void main(String[] args) {
		...
		JFrame login = new LoginDialog();// 实列化登录窗体
	 ...
	 * @return JToolBar
	 */
	private ToolBar getJJToolBar() {//初始化工具栏（√）
	 ...
	 * @return JMenuBar
	 */
	protected MenuBar getFrameMenuBar() {//初始化窗体菜单栏的方法（√）
	 ...
	 * @return JDesktopPane
	 */
	private DesktopPanel getDesktopPane() {//初始化桌面面板的方法（√）
	...
	private JSeparator getJSeparator1() {// 获得分隔符1（√）
	...
	private JSeparator getJSeparator2() {// 获得分隔符2（√）
	...
	private JPanel getFrameContentPane() {// 获得内容面板的方法（√）
		if (frameContentPane == null) {// 内容面板对象为空
			...
			frameContentPane.add(getDesktopPane(), CENTER);// 桌面面板置于内容面板的南部
		}
