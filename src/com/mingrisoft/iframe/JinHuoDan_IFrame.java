...
public class JinHuoDan_IFrame extends JInternalFrame {// 进货单窗体
	...
	private JPanel getJContentPane() {//获得内容面板的方法
	...
	private JButton getTjButton() {//获得“添加”按钮的方法(√)
	...
	private void initSpBox() {//初始化商品下拉框(√)
		List<String> list=new ArrayList();
		ResultSet set=Dao.query("select * from tb_spinfo where gysName='" + gysComboBox.getSelectedIndex() + "'");
		spComboBox.removeAllItems();//如果是用addItem()添加的,就用removeAllItems()移出。
		spComboBox.addItem(new TbSpinfo());//用addItem()添加商品信息
		for(int i=0;table!=null&&i<table.getRowCount();i++) {//table 表格模型 getRowCount获取行数量
			TbSpinfo tmplnfo = (TbSpinfo) table.getValueAt(i, 0);//getValueAt(introw,intcolumn)返回row和column位置的单元格值。
			if(tmplnfo!=null&&tmplnfo.getId()!=null) {
				list.add(tmplnfo.getId());
			}
		}
		try {
			while(set.next()) {
			TbSpinfo spinfo=new TbSpinfo();//sp信息
			spinfo.setId(set.getString("id").trim());
			//如果表格中已存在同样的商品，商品下拉框就不再添加该商品
	...
	private JComboBox getSpComboBox() {//获得商品下拉列表的方法（√）
	...
	private synchronized void updateTable() {//根据商品下拉列表的选择，更新表格当前行的内容
	...
	private void updateSpComboBox(ResultSet set) {//更新商品下拉列表的方法
		try {
	...
	private JButton getRukuButton() {//获得“入库”按钮的方法
		if (rukuButton == null) {// 如果“入库”按钮不存在
			...
			rukuButton.addActionListener(new java.awt.event.ActionListener() {// 为“入库”按钮添加动作事件的监听
				public void actionPerformed(java.awt.event.ActionEvent e) {
					stopTableCellEditing();// 停止表格单元的编辑
					clearEmptyRow();// 清除空行
					...
					TbRukuMain ruMain = new TbRukuMain(id, pzsStr, jeStr, ysjlStr, gysName, rkDate, 
							czyStr, jsrStr,jsfsStr);// 入库主表
	...
	private JComboBox getGysComboBox() {//供应商
	...
	private final class computeInfo implements ContainerListener {//事件处理器，该处理器用于计算货品总数、合计金额等信息。
	...
	private JTable getTable() {//获得总表格模型的方法(√)
		if (table == null) {// 如果表格模型不存在
			...
			table.addPropertyChangeListener(new PropertyChangeListener() {//监听属性改变事件
	...
	private JTextField getIdField() {//获得“进货票号”文本框的方法
	...
	private JComboBox getJsfsComboBox() {//获得“结算方式”文本框的方法
	...
	private JTextField getJhsjField() {//获得“进货时间”文本框的方法
	...
	private JTextField getCzyField() {//获得“操作员”文本框的方法
	...
	private JTextField getPzslField() {//获得“品种数量”文本框的方法
	...
	private JTextField getHpzsField() {//获得“货品总数”文本框的方法
	...
	private JTextField getHjjeField() {//获得“合计金额”文本框的方法
	...
	private JTextField getYsjlField() {//获得“验收结论”文本框的方法
	...
	private void stopTableCellEditing() {//该方法用于停止表格单元的编辑
	...
	private JScrollPane getTablePane() {// 获得表格面板的方法
	...
	private JPanel getBottomPanel() {//获得底部面板的方法（内含组件）
	...
	private JPanel getTopPanel() {//获得顶部面板的方法
